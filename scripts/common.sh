...
function patch_mac_build {
    if [[ "$OS" == "Darwin" ]]; then
        export APP_DIR="$BASE_DIR/build/Boing.app"
        ...
        if [[ -d "$APP_DIR" ]]; then
            ...
            # CREATE WRAPPER
            mv "$APP_MACOSX_DIR/Boing" "$APP_MACOSX_DIR/Boing.bin"

            tee "$APP_MACOSX_DIR/Boing" <<"EOF"
#!/usr/bin/env bash
MY_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}")" && pwd )"
(cd "$MY_DIR" && ./Boing.bin)
EOF
            chmod 0755 "${APP_DIR}/Contents/MacOS/Boing"
         
            # INSTALL APP.PLIST & ETC
            cp "$BASE_DIR/resources/info.plist" "$APP_CNT_DIR"
            mkdir -p "$APP_RES_DIR"
            cp "$BASE_DIR/resources/desktop-icon/Boing.icns" "$APP_RES_DIR"
...
function checkout_creative_engine_branch {
    ...
    BOING_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    cd "$BASE_DIR" || exit 1
    echo "The current modite branch is: $BOING_BRANCH"
    if ($BOING_BRANCH != "master" && cd "$CREATIVE_ENGINE_DIR" && git checkout "$BOING_BRANCH"); then
        echo "Checked out creatine-engine branch: $BOING_BRANCH"
...
function archive_app {
    ...
    if [[ "${OS:-}" == "Darwin" ]]; then
        ...
        tar czvfp "./$ARCHIVE_NAME" Boing.app
        cd - || exit 1
    elif [[ "${OS:-}" == "Linux" ]]; then
